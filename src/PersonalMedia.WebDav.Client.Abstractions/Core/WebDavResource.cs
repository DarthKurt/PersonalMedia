...
public class WebDavResource
{
    ...
    public class Builder
    {
        ...
        {
            Guard.IsNotNull(propertyStatuses);
        ...
        public WebDavResource Build()
            => new()
            {
                ActiveLocks = _activeLocks ?? Array.Empty<ActiveLock>(),
                ContentLanguage = _contentLanguage ?? "unknown",
                ContentLength = _contentLength,
                ContentType = _contentType ?? "unknown",
                CreationDate = _creationDate,
                DisplayName = !string.IsNullOrWhiteSpace(_displayName)
                    ? _displayName
                    : ThrowHelper.ThrowInvalidOperationException<string>("Invalid item received. Display name should be not null or empty"),
                ETag = _eTag,
                Uri = _uri,
                IsCollection = _isCollection,
                IsHidden = _isHidden,
                LastModifiedDate = _lastModifiedDate,
                Properties = _properties ?? Array.Empty<WebDavProperty>(),
                PropertyStatuses = _propertyStatuses ?? Array.Empty<WebDavPropertyStatus>()
            };
    }
}